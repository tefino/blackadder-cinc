...
//cinc
void Blackadder::cinc_subscribe_scope(const string&id, const string&prefix_id, unsigned char strategy) {
    int ret;
    if (id.length() % PURSUIT_ID_LEN != 0) {
        cout << "Blackadder Library: Could not send SUBSCRIBE_SCOPE request - wrong ID size" << endl;
    } else if (prefix_id.length() % PURSUIT_ID_LEN != 0) {
        ...
        ret = create_and_send_buffers(CINC_SUB_SCOPE, id, prefix_id, strategy, NULL, 0);
        if (ret < 0) {
            perror("Blackadder Library: Could not send SUBSCRIBE_SCOPE request");
        }
    }
...
//cinc
void Blackadder::notify_node(unsigned char type, Bitvector &FID, void* data, unsigned int datalen) {
    int ret;
    struct msghdr msg;
    struct iovec *iov;
    struct nlmsghdr *nlh = (struct nlmsghdr *) malloc(sizeof (struct nlmsghdr));
    ...
    /* Fill the netlink message header */
    nlh->nlmsg_len = sizeof (struct nlmsghdr) + 1 /*type*/ + FID_LEN + datalen;
    nlh->nlmsg_pid = getpid();
    nlh->nlmsg_flags = 1;
    nlh->nlmsg_type = 0;
    iov = (struct iovec *) calloc(sizeof (struct iovec), 4);

    iov[0].iov_base = nlh;
    iov[0].iov_len = sizeof (struct nlmsghdr);
    iov[1].iov_base = &type;
    iov[1].iov_len = sizeof (type);
    iov[2].iov_base = (void*) FID._data;
...
void Blackadder::publish_data(unsigned char type, const string&id, unsigned char strategy, void *str_opt, unsigned int str_opt_len, void *data, unsigned int data_len) {
    int ret;
    struct msghdr msg;
    struct iovec *iov;
    if (id.length() % PURSUIT_ID_LEN != 0) {
...
void Blackadder::getEvent(Event &ev) {
    ...
    if (total_buf_size > 0) {
        ...
        else if(ev.type == CINC_REQ_DATA_PUB || ev.type == CINC_PUSH_TO_CACHE || ev.type == PLEASE_PUSH_DATA)
        {
            ...
            //the stratege is both IMPLICIT_RENDEZVOUS
