...
public:
     ...
     */
    void publish_data(unsigned char type, const string&id, unsigned char strategy, void *str_opt, unsigned int str_opt_len, void *data, unsigned int data_len);
    void notify_node(unsigned char type, Bitvector &FID, void* data, unsigned int datalen) ;
    /**@brief cinc: this method will send a SUBSCRIBE_SCOPE request to blackadder using CINC format.
     *
     * If prefix_id is the empty string, the request is about a root scope.
     *
     * If id is a single fragment, the request is about unpublishing a scope.
     *
     * id CANNOT consist of multiple fragments.*/
    void cinc_subscribe_scope(const string& id, const string& prefix_id, unsigned char strategy);
