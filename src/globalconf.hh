...
public:
    ...
    /**@brief hash function
     */
    int cinc_hash(String &input, int numofround, Vector<unsigned int> &value);
    /**@brief hash function, fast, fair randomness*/
    unsigned int fnv1a(const void* data, unsigned int numBytes, unsigned int& hash) ;
    ...
    /**@brief cinc: the number of routers in this area
     */
    int num_router ;
    /**@brief cinc: the number of the node in the area
     */
    int nodeNUM ;
    /*cinc:*/
    uint32_t cache_degree ;
    uint32_t popthreshold ;
    uint32_t interval ;
};
