...
public:
    ...
    /**@brief cinc: This method send req to local publisher*/
    void AskPubPushData(unsigned char type, Packet* p) ;
    /**@brief cinc: after receive FID information from TM, the LP first store these info*/
    bool cinc_saveFIDinfo(Vector<String>& IDs, unsigned int index2fidinfo, Packet* p) ;
    /**@brief cinc: send request to publisher or cache router*/
    void cinc_sendrequest(Vector<String>& IDs, unsigned int index, Packet* p) ;
    /**@brief cinc: handle the subscription request, only support scopr subscription*/
    void cinc_handlesubscription(String ID, Packet* p, BABitvector& RVFID) ;
    /**@brief cinc: push data to router for caching*/
    void cinc_pushDATAtoRouter(String &ID, BABitvector &FID_to_subscribers, Packet *p, LocalHost *_localhost);
